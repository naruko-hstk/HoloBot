...
const commandCore = fs
  .readdirSync('./Extension/Core')
  .filter((file) => file.endsWith('.js'))
const commandCustom = fs
  .readdirSync('./Extension/Custom')
  .filter((file) => file.endsWith('.js'))
const commandError = fs
  .readdirSync('./Extension/Error')
  .filter((file) => file.endsWith('.js'))
const commandEvents = fs
  .readdirSync('./Extension/Events')
  .filter((file) => file.endsWith('.js'))
const commandFun = fs
  .readdirSync('./Extension/Fun')
  .filter((file) => file.endsWith('.js'))
const commandModerations = fs
  .readdirSync('./Extension/Moderations')
  .filter((file) => file.endsWith('.js'))
const commandUtilities = fs
  .readdirSync('./Extension/Utilities')
  .filter((file) => file.endsWith('.js'))
...
holo.on('message', (msg) => {
  ...
  //Command with prefix
    ...
    if (
      msg.author.id === '277389659947008001' ||
      msg.author.id === '487804795902492712'
    )
  ...
  else if (command === 'fragments') {
    if (!args.length) holo.commands.get('fragments').run(holo, msg, connection)
    else if (args[0] === add) holo.commands.get('fragmentsAdd').execute(msg)
  } else if (command === 'cp') {
    if (!args.length) holo.commands.get('cp').run(holo, msg, connection)
    else if (args[0] === 'cpAdd') holo.commands.get('cpadd').execute(msg)
  } else {
    return
  }
})
