...
module.exports = {
	...
	async execute(msg, args, prefix, command, author, master, connection) {
		...
		if (args[0] === 'easy') {
			await connection.query(`SELECT SongName FROM chaosjudge.cytusii WHERE EasyLevel = ${parseInt(args[1])};`, (err, rows) => {
				if (err) throw err;
			...
			await connection.query(`SELECT SongName FROM chaosjudge.cytusii WHERE HardLevel = ${parseInt(args[1])};`, (err, rows) => {
				if (err) throw err;
				if (rows.length < 1) msg.channel.send(nodata);
				else {
					list.push(`共有**${rows.length}**首曲子：\n`);
			...
			await connection.query(`SELECT SongName FROM chaosjudge.cytusii WHERE ChaosLevel = ${parseInt(args[1])};`, (err, rows) => {
				if (err) throw err;
				if (rows.length < 1) msg.channel.send(nodata);
				else {
					list.push(`共有**${rows.length}**首曲子：\n`);
			...
			await connection.query(`SELECT SongName FROM chaosjudge.cytusii WHERE GlitchLevel = ${parseInt(args[1])};`, (err, rows) => {
				if (err) throw err;
				if (rows.length < 1) msg.channel.send(nodata);
				else {
					list.push(`共有**${rows.length}**首曲子：\n`);
