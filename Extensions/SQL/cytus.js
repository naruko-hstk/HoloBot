...
module.exports = {
	...
	execute(msg, args, prefix, command, author, master, connection) {
		...
		if (args[0] === 'easy') {
			connection.query(`SELECT SongName FROM chaosjudge.cytus WHERE EasyLevel = ${parseInt(args[1])};`, (err, rows) => {
				if (err) throw err;
			...
			connection.query(`SELECT SongName FROM chaosjudge.cytus WHERE HardLevel = ${parseInt(args[1])};`, (err, rows) => {
				if (err) throw err;
				if (rows.length < 1) msg.channel.send(nodata);
				else {
					list.push(`共有**${rows.length}**首曲子：\n`);
